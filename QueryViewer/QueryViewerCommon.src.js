...
qv.chart = (function () {
	...
	function TooltipFormatter(evArg, chartSeries, sharedTooltip)
	{
		...
		if (!sharedTooltip) {
			...
			return '<b>' + (evArg.point.name != "" ? evArg.point.name : evArg.series.name) + '</b>: ' + qv.util.formatNumber(evArg.point.y, decimalPrecision, picture, removeTrailingZeroes) + (IsGaugeChart(qViewer) ? "%" : "");
		}
		else {
			var firstPoint;
			var index;
	...
	function DateTimeTooltipFormatter(evArg, chartSeries) {
		...
		for (var strIndex in points_by_strIndex) {
			...
			for (var ind = 0; points[ind] != undefined; ind++) {
				...
				var keySpan = qv.util.dom.createSpan(null, "", "", "", { }, null, (compare ? p.name : seriesName) + ": ").outerHTML;
